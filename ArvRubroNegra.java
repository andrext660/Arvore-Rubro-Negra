...
public class ArvRubroNegra<Chave extends Comparable<Chave>,Valor> {
	...
	public void imprimir(ArvRubroNegra<Chave, Valor> arvore) {
		impElemen(arvore.raiz);
	}

	private void impElemen(NoRB<Chave, Valor> no) {
		if (no != null) {
			impElemen(no.getNoEquerdo());

			System.out.println(no.getElemento());
			impElemen(no.getNoDireito());
		}
	}
    ...
    private static final boolean Preto = false;
     ...
     private NoRB minimo(NoRB no) { 
         assert no != null;
         if (no.getNoEquerdo() == null) return no; 
         else 
        return minimo(no.getNoEquerdo()); 
     } 
     
  // the largest key; null if no such key
     public Comparable max() {
         if (isEmpty()) return null;
         return max(raiz).getChave();
     } 

     // the largest key in the subtree rooted at x; null if no such key
     private NoRB max(NoRB no) { 
         assert no != null;
         if (no.getNoDireito() == null) return no; 
         else                
        	 return max(no.getNoDireito()); 
     } 
...
//Deletar o valor de chave minima
 ...
 /**** Rotacionar Árvore para direita*****/
	 ...
	 private void inversaoDeCores(NoRB no) {
		 
		 assert (no!=null) && (no.getNoEquerdo() !=null) && (no.getNoDireito() !=null);
		 assert (!eVermelho(no)) && eVermelho(no.getNoEquerdo()) && eVermelho(no.getNoDireito())
		 || (eVermelho(no) && !eVermelho(no.getNoEquerdo()) && !eVermelho(no.getNoDireito()));
		 no.cor = !no.cor;
		 no.getNoEquerdo().cor = !no.getNoEquerdo().cor;
		 no.getNoDireito().cor = !no.getNoDireito().cor;
	 }
 ...
 * Transforma no.esquerda ou um dos seus filhos em vermelho */
	  ...
	  * Transforma no.esquerda ou um dos seus filhos em vermelho */
	 	...
	 	// Altura da Árvore (1-node tree has height 0)
	     
	 
	 	 public int height() { 
	         return height(raiz); 
	     }
	     private int height(NoRB x) {
	         if (x == null) return -1;
	         return 1 + Math.max(height(x.getNoEquerdo()), height(x.getNoDireito()));
	     }
